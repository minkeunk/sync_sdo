...
static void* _download_today_data(void *arg)
{
    ...
    struct tm *today_s, *start_s, *end_s;
    FILE *fp;

    printf("downloading today\n");
    do {
        start = time(NULL);
        start_s = gmtime(&start);
        sprintf(logfile, "%04d%02d%02d.log", start_s->tm_year+1900,
                    start_s->tm_mon+1, start_s->tm_mday);
        //fp = open_log(logfile);
        fp = (FILE*)stderr;
...
static void* _download_past_data(void *arg)
{
    ...
    do {
        ...
        list_for_each_safe(pos, q, &(work_list->list)) {
            ...
            today_s = gmtime(&today);

            if ((today_s->tm_year != tm_s->tm_year) &&
                (today_s->tm_mon != tm_s->tm_mon) &&
                (today_s->tm_mday != tm_s->tm_mday))
                continue;
